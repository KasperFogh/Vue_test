...
namespace ModelsApi.Controllers
{
    ...
    public class AccountController : ControllerBase
    {
        ...
        public async Task<ActionResult<Token>> register([FromBody] Register register)
        {
            if (register != null)
            {
                register.Email = register.Email.ToLowerInvariant();
                EfAccount account = new EfAccount() { Email = register.Email, IsManager = false, PwHash = register.Password };
                _context.Accounts.Add(account);
                await _context.SaveChangesAsync().ConfigureAwait(false);
                return CreatedAtAction("GetAccount", new { id = account.EfAccountId }, register);
